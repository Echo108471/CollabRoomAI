...
<div class="flex h-dvh bg-surface-50 text-slate-800 font-sans overflow-hidden">
	<!-- Sidebar -->
	<aside
		class="w-64 bg-surface-900 text-slate-300 flex flex-col border-r border-slate-800 shadow-xl z-20"
	>
		...
		<div class="flex-1 overflow-y-auto py-4">
			<div class="px-4 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
				Rooms
			</div>
			<nav class="space-y-0.5 px-2">
				<button
					class="w-full text-left px-3 py-2 rounded-md bg-slate-800 text-white font-medium flex items-center gap-2 transition-colors"
				>
					<span class="text-slate-400">#</span>
					{roomId}
				</button>
				<button
					class="w-full text-left px-3 py-2 rounded-md hover:bg-slate-800/50 text-slate-400 hover:text-slate-200 transition-colors flex items-center gap-2"
				>
					<span class="text-slate-500">#</span> random
				</button>
			</nav>

			<div class="px-4 mt-8 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
				Team
			</div>
	...
	<div class="flex-1 flex flex-col min-w-0 bg-white relative">
		<!-- Glassmorphic Header -->
		<header
			class="h-16 px-6 flex items-center justify-between bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-10"
		>
			<div class="flex items-center gap-3">
				<span class="text-xl font-bold text-slate-800"># {roomId}</span>
				<span
					class="px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-xs font-medium border border-slate-200"
			...
			<div class="flex -space-x-2">
				...
				<div
					class="w-8 h-8 rounded-full bg-purple-100 border-2 border-white flex items-center justify-center text-xs font-bold text-purple-600"
					title="SkepticBot"
				>
					SB
				</div>
		...
		<footer class="p-6 bg-white border-t border-slate-100">
			<div class="max-w-4xl mx-auto">
				<MessageInput on:send={(e: CustomEvent) => onSend(e.detail)} />
			</div>
		</footer>
	</div>
</div>
